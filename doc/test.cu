...
__global__ void dslash(void *device_U, void *device_src, void *device_dest,
                       ...
                       const int parity) {
  __shared__ Complex warp_output_vec[BLOCK_SIZE * NS * NC];
  int warp_thread_rank = threadIdx.x & (WARP_SIZE-1);
  int warp_pos = threadIdx.x >> 5;
  Complex* shared_dest;
  shared_dest = static_cast<Complex*>(b_ptr) + blockIdx.x * BLOCK_SIZE * NS * NC + warp_pos * WARP_SIZE * NS * NC;
  ...
  LatticeComplex *origin_U =
      ((static_cast<LatticeComplex *>(device_U)) + t * local_lat_zyxcc +
       z * local_lat_yxcc + y * local_lat_xcc + (x + parity * local_lat_x) * 9);
  ...
  // }
  {
    ...
    local_U[6] = local_U[6].conj();
    local_U[7] = local_U[7].conj();
    local_U[8] = local_U[8].conj();
...
//   }
    ...
    for (int i = 0; i < NS * NC; i++) {
        warp_output_vec[threadIdx.x * NS * NC + i] = local_dest[i];
    }
    __syncthreads();
    // load to global memory
    for (int i = warp_thread_rank ; i < WARP_SIZE * NS * NC; i += WARP_SIZE) {
        shared_dest[i] = warp_output_vec[warp_pos * WARP_SIZE * NS * NC + i];
    }
    __syncthreads();
  // dest = local_dest;
