...
void dslashQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  int lat_x = param->lattice_size[0] / 2;
...
//   checkCudaErrors(
//       cudaMemcpy(device_src, fermion_in, fermi_size,
...
//   for (int i = 0; i < loop; i++) {
//     dslash<<<gridSize, blockSize>>>(
//         device_U, device_src, device_dest, lat_x, lat_y, lat_z, lat_t,
//         lat_xcc, lat_yxcc, lat_zyxcc, lat_tzyxcc, lat_xsc, lat_yxsc,
//         lat_zyxsc, parity);
//     checkCudaErrors(cudaDeviceSynchronize());
//   }
//   clock_t end0 = clock();
//   checkCudaErrors(
//       cudaMemcpy(fermion_out, device_dest, fermi_size,
...
//             << (double)(end1 - start) / CLOCKS_PER_SEC << "s" <<
//             std::endl;
