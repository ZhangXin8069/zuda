...
public:
  ...
  */
  ...
  __device__ __host__ inline Cholesky(const matrix_t &A) {
    const Mat<T, N> &L = L_;
...
#pragma unroll
          ...
          if (!fast) { // traditional Cholesky with sqrt and division
            L_(i, j) = (i == j) ? sqrt((A_ij - s).real())
                                : (A_ij - s) / L(j, j).real();
          } else { // optimized - since fwd/back subsitition only need inverse
                   // diagonal elements, avoid division and use rsqrt
            L_(i, j) = (i == j) ? quda::rsqrt((A_ij - s).real())
                                : (A_ij - s) * L(j, j).real();
          }
        }
    }
  ...
  */
  __device__ __host__ inline const T D(int i) const {
    ...
    if (!fast)
      ...
      return static_cast<T>(1.0) / L(i, i).real();
  }
...
#pragma unroll
      ...
      if (!fast)
        x(i) /= L(i, i).real(); // traditional
      else
        x(i) *= L(i, i).real(); // optimized
    }
    return x;
  }
...
#pragma unroll
  ...
  /**
     @brief Compute the inverse of A (the matrix used to construct the Cholesky
     decomposition).
...
#pragma unroll
    for (int k = 0; k < N; k++) {
      ...
      // forward substitute
      if (!fast)
        v(k) = complex<T>(static_cast<T>(1.0) / L(k, k).real());
      else
        v(k) = L(k, k).real();
...
#pragma unroll
      ...
      // backward substitute
      if (!fast)
        v(N - 1) *= static_cast<T>(1.0) / L(N - 1, N - 1);
      else
        v(N - 1) *= L(N - 1, N - 1);
